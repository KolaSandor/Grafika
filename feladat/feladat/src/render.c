...
static void draw_tree(float x, float z) {
    ...
    glRotatef(-90, 1, 0, 0);
    gluCylinder(cyl, 1.0f, 0.9f, 9.0f, 13, 5);
    glPopMatrix();
...
void draw_rain() {
    glDisable(GL_LIGHTING);
    glColor4f(1.0f,1.0f,1.0f, 1.0f);  // Világoskék, félig átlátszó
    glLineWidth(2.0f);
    glBegin(GL_LINES);
    for (int i=0; i<RAIN_COUNT; ++i) {
        glVertex3f(rain[i].x, rain[i].y, rain[i].z);
        glVertex3f(rain[i].x, rain[i].y - 3.5f, rain[i].z);  // Esőcsepp hossza
    }
    glEnd();
    glEnable(GL_LIGHTING);
}
                  ...
                  SDL_Window*   window)
{
    ...
    glEnable(GL_LIGHTING);
    glEnable(GL_LIGHT0);
    extern float lightIntensity; // input.c-ből

    {
      GLfloat pos[]  = {0,10,10,1};
      GLfloat amb[]  = {0.2f * lightIntensity, 0.2f * lightIntensity, 0.2f * lightIntensity, 1};
      GLfloat dif[]  = {0.8f * lightIntensity, 0.8f * lightIntensity, 0.8f * lightIntensity, 1};
      GLfloat spec[] = {1.0f * lightIntensity, 1.0f * lightIntensity, 1.0f * lightIntensity, 1};
    ...
    // --- FORGÓ NAP SUGARAKKAL ---
    {
        ...
        // nap és sugarak méretei
        const float sunR   = 12.0f;  // nap sugara
        const float rayLen = 35.0f;  // sugarak hossza
        const int   rays   = 12;     // sugarak száma

        sunAngle += 0.5f;
        if (sunAngle >= 360.0f) sunAngle -= 360.0f;

        // nap pozíciója az autóhoz képest
        ...
        glPushMatrix();
          ...
          // Y tengely körüli forgatás a sugarak körbeforgatásához
    ...
    // --- HÁTTÉR-HEGYEK, SZÍN ÉS MAGASSÁG VÁLTOZÁSSAL ---
    {
        ...
        // Előtér-hegyek (sötétebb, magasabb, kicsit random)
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        float wing = 25.0f*sinf(t*2.0f + i);
